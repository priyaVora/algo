...
public class DijkstraAlgorithm<T> {
	...
	public DijkstraAlgorithm<String> makeTestData() {
		...
		nodeA.addDestination(nodeD, nodeD.getDistance());
	...
	public void DijkstraCaller() {
		System.out.println(" Source: " + this.sourceNode);
		currentNode = sourceNode;
		System.out.println(" 	Current: " + this.currentNode);
		this.DijkstraAlgo(this.currentNode);
	}

	public void DijkstraAlgo(Node<T> currentNode) {
		System.out.println(" Source: " + this.sourceNode);
		currentNode = sourceNode;
		System.out.println(" 	Current: " + currentNode);

		leftOverNode.remove(currentNode);
		System.out.println("PRINT AFTER REMOING THE CURRENT NODE " + this.currentNode.getName());
		System.out.println("\n");
		...
		loop: while (it.hasNext()) {
			...
			System.out.print("Neighbors to " + currentNode.getName() + ":  " + temp.getName());
			List<String> nodesList = this.valuesFromLeftOverNodes(leftOverNode);
			if (!(nodesList.contains(temp.getName()))) {
				continue;
			}
			// int weight = getSpecificEdgeWeight_src_Destination(currentNode.getName(),
			// temp.getName());
			//
			// if (nodesList.contains(temp.getName())) {
			// int v = leftOverNode.get(temp);
			// leftOverNode.remove(temp);
			// this.removeFromLeftOver(leftOverNode);
			// leftOverNode.put(temp, weight);
			// temp.setDistance(weight);
			// System.out.println("UPDATED: " + leftOverNode.get(temp));
			//
			//
			// ///
			// if( ) {
			//
			// }
			// pathMap.put(temp, this.sourceNode);
			// }
		}

		if (!(leftOverNode.isEmpty())) {
			this.currentNode = extraMin();
			distanceMap.put(this.currentNode, this.currentNode.getDistance());
			this.printData();
			DijkstraAlgo(this.currentNode);
		}

	}

	public List<Node<T>> valuesFromDistanceMap(Map<Node<T>, Integer> x) {
		List<Node<T>> list = new ArrayList<Node<T>>();
		Iterator<Entry<Node<T>, Integer>> n = x.entrySet().iterator();
		Iterator it = n;
		while (it.hasNext()) {
			Map.Entry pair = (Map.Entry) it.next();
			Node<T> temp = (Node<T>) pair.getKey();
	...
	public List<String> valuesFromLeftOverNodes(Map<Node<T>, Integer> map) {
		System.out.println(" ");
	...
	public void hashMapsNodes(Map<Node<T>, Node<T>> x) {
		...
		while (it.hasNext()) {
			Map.Entry pair = (Map.Entry) it.next();
			Node<T> relation = (Node<T>) pair.getValue();
			Node<T> temp = (Node<T>) pair.getKey();
			if (temp.getName() == this.sourceNode.getName()) {
				temp.setDistance(null);
			}
			System.out.println(temp.getName() + " = " + relation);
			// it.remove(); he is just there
		}
	}
}
