...
public class Sorter<T extends Comparable<? super T>> {
	...
	private static void merge(Comparable[] dataArray, int startingPoint, int mergeSortMiddle, int endPoint) {
		Comparable[] dataHolder = new Comparable[dataArray.length];
		for (int i = startingPoint; i <= endPoint; i++) {
			dataHolder[i] = dataArray[i];
		}

		int leftSide = startingPoint;
		int rightSide = mergeSortMiddle + 1;

		relocateValuesHelper(dataArray, dataHolder, startingPoint, mergeSortMiddle, endPoint, leftSide, rightSide);
	}

	private static void relocateValuesHelper(Comparable[] dataArray, Comparable[] dataHolder, int startingPoint,
			int mergeSortMiddle, int endPoint, int leftSide, int rightSide) {
		for (int currentPositionOfList = startingPoint; currentPositionOfList <= endPoint; currentPositionOfList++) {

			int farRightSideValue = mergeSortMiddle + 1;

			if (leftSide == farRightSideValue) {
				dataArray[currentPositionOfList] = dataHolder[rightSide++];
			} else if (rightSide > endPoint) {
				dataArray[currentPositionOfList] = dataHolder[leftSide++];
			} else if (dataHolder[leftSide].compareTo(dataHolder[rightSide]) < 0) {
				System.out.println("Value: " + dataHolder[leftSide].compareTo(dataHolder[rightSide]));
				dataArray[currentPositionOfList] = dataHolder[leftSide++];
			} else {
				dataArray[currentPositionOfList] = dataHolder[rightSide++];
			}
